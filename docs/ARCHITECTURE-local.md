...
flowchart TB
    ...
    CC -- "MCP tools: search_facts · get_context ·<br/>list_namespaces · propose_fact · submit_promotion ·<br/>list_my_promotions · review_promotion · save_memory<br/>(HTTP /mcp, X-Baseline-Principal)" --> SVC
...
| Claude Code (MCP)       | baseline `:8080/mcp`     | MCP-over-HTTP, `X-Baseline-Principal`                    | `search_facts` (semantic), `get_context`, `list_namespaces`, `propose_fact`, `submit_promotion`, `list_my_promotions`, `review_promotion`, `save_memory` |
